...
public class EventHandler implements OnClickListener {
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.back_button:
                if (mFileMang.getCurrentDir() != "/") {
                    if (multi_select_flag) {
                        ...
                        Toast.makeText(mContext, "Multi wybiarnie wyłączone",
            ...
            case R.id.home_button:
                if (multi_select_flag) {
                    mDelegate.killMultiSelect(true);
                    Toast.makeText(mContext, "Multi wybieranie wyłączone",
                            Toast.LENGTH_SHORT).show();
                }
            ...
            case R.id.hidden_delete:
                ...
                builder.setMessage("Czy na pewno chcesz usunąć " +
                        data.length + " pliki? To niemoże być " +
                        "cofnięte.");
    ...
    private void display_dialog(int type) {
        ...
        switch (type) {
            case MANAGE_DIALOG:
                ...
                CharSequence[] options = {"Informacje o procesach", /*"Wifi Info",*/ "Kopia zapasowa aplikacji"};

                builder = new AlertDialog.Builder(mContext);
                builder.setTitle("Narzędzia");
    ...
    private class BackgroundWork extends AsyncTask<String, Void, ArrayList<String>> {
        ...
        protected void onPreExecute() {
            ...
            switch (type) {
                case SEARCH_TYPE:
                    pr_dialog = ProgressDialog.show(mContext, "Szukanie",
                            "Wyszukiwanie w bieżącym systemie plików...",
                            true, true);
                    break;

                case COPY_TYPE:
                    pr_dialog = ProgressDialog.show(mContext, "Kopiowanie",
                            "Kopiowanie plików...",
                            true, false);
                    break;

                case UNZIP_TYPE:
                    pr_dialog = ProgressDialog.show(mContext, "Rozpakowywanie",
                            "Rozpakowaniu pliku zip proszę czekać...",
                            true, false);
                    break;
                ...
                case ZIP_TYPE:
                    pr_dialog = ProgressDialog.show(mContext, "Pakowanie",
                            "Pakowanie folderu...",
                            true, false);
                    break;

                case DELETE_TYPE:
                    pr_dialog = ProgressDialog.show(mContext, "Usuwanie",
                            "Usuwanie plików...",
                            true, false);
                    break;
            }
        ...
        protected void onPostExecute(final ArrayList<String> file) {
            ...
            switch (type) {
                case SEARCH_TYPE:
                    if (len == 0) {
                        Toast.makeText(mContext, "Niemożna znaleźć " + file_name,
                                Toast.LENGTH_SHORT).show();
                        ...
                        AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
                        builder.setTitle("Znaleziono " + len + " plików");
                ...
                case COPY_TYPE:
                    ...
                    if (copy_rtn == 0)
                        Toast.makeText(mContext, "Plik został skopiowany i wklejony",
                                Toast.LENGTH_SHORT).show();
                    else
                        Toast.makeText(mContext, "Błąd kopiowania", Toast.LENGTH_SHORT).show();
